...
- (NSString *) stringFromDate:(NSDate *)date formatString:(NSString *)dateFormat timeZone:(NSTimeZone *)timeZone {
	if (includeTime){
		NSString *timeFormat = ISO_TIME_FORMAT;
    ...
    if (useMillisecondPrecision){
        NSString *microsecondString = [[NSString stringWithFormat:@"%.6f", [date timeIntervalSince1970]] componentsSeparatedByString:@"."][1];
        str = [NSString stringWithFormat:@"%@.%@", str, [microsecondString copy]];
        [microsecondString release];
    }
...
- (NSString *) weekDateStringForDate:(NSDate *)date timeZone:(NSTimeZone *)timeZone {
	...
	if(includeTime) {
        ...
        if (useMillisecondPrecision){
            NSString *microsecondString = [[NSString stringWithFormat:@"%.6f", [date timeIntervalSince1970]] componentsSeparatedByString:@"."][1];
            timeString = [NSString stringWithFormat:@"%@.%@", timeString, [microsecondString copy]];
            [microsecondString release];
        }
