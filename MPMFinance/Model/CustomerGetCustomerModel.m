...
+ (void)getListCustomerGetCustomerCompletion:(void(^)(NSArray *responses, NSError *error))block{
    ...
    [manager POST:[NSString stringWithFormat:@"%@/customergetcustomer/getall",kApiUrl] parameters:param progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        @try {
            NSInteger code = [[responseObject objectForKey:@"statusCode"] integerValue];
            NSString *message = [responseObject objectForKey:@"message"];
            if (code == 200) {
